...
**Agent A — Feedback** (`name: "squad-identity-feedback"`):
```
...
**Agent B — Profile** (`name: "squad-identity-profile"`):
```
...
**Agent C — Projects** (`name: "squad-identity-projects"`):
```
...
**Agent D — Sessions** (`name: "squad-identity-sessions"`):
```
...
<!-- generated by /squad-identity on [TODAY'S DATE] -->
...
**Session carve-outs (from CLAUDE.md):** add your own carve-outs with `/squad-proxy <exception>`.
...
**Save:** Write the synthesis agent's output to `~/.claude/skills/squad-proxy/${LOGNAME}-identity.md`.
...
- Suggestion: "Run `/squad-proxy` to activate your proxy with the updated identity."
